...
| F22 | **Territory maps splitting one region across territories silently collapsed** (dict last-one-wins) - half the plan units got zero pipeline, attainment pinned at 46.6% | engine splits multi-territory regions evenly via named stream [seed, 6] |
| F23 | **Drafter omits quota blocks required by coverage_ratio criteria, and the loop cannot add them** (plan-of-record) - session dead-ends | preflight flags it HARD; auto-calibration SYNTHESIZES a naturally-sized quota block deterministically; prompt updated to list coverage_ratio |
