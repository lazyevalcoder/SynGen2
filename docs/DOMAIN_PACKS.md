...
| P6 ✅ | **Realizability gate + missing surfaces** (branch `fix/realizability`, 3 commits) - the "criteria signed that the generator cannot truthfully construct" class (findings_v3.md F19.x). Part 1 (gate): unknown/hallucinated check names ⇒ hard finding + bounded redraft (F19.8); engine defaults `accounts.segments` when absent (F19.7); geometry lint gets one corrective criteria re-draft with expressible-form hints before escalating (F19.6); tier revenue-share targets above the arithmetic ceiling flagged infeasible (F19.3). Part 2 (surfaces): capacity synthesis rises headcount flow when `headcount_growth_placement` required + converge remedy concentrates additions into measured-strong units before the structural check (F19.10); elasticity_differential solver sets price path/elasticity + deal-count floor to land the gap deterministically (F19.9/AC6); `quota_vs_potential` cohort expressions (`top_pct`/`bottom_pct`) supported in check+signature+remedy (F19.6, kills s15/s18 subset claims). Part 3 (honesty net): calibration-agreement sweep test - every closed-form solver run against a real workbook across 3 seeds (drift gate); deal-count floor for noise-sensitive margin/share/elasticity checks (F19.2); margin noise-floor lint note (~4pp tail). All rules are general (keyed to check names/config shapes, zero scenario conditionals). Suite 372 green. Verification: fresh cohort 21-25 is the real test (never examined); landing set 05/09/11/13/14 preservation is a regression check. Record: `findings_v4.md`. **MERGED to master 2026-09-09; preservation re-fly 5/5 LANDED (05/09/11/13/14).** |
| P7 ✅ | **Flexible stage-rework loop** (same branch) - replaces the rigid one-way door with a bounded, LLM-judged feedback loop: a delivery escalation no longer dies terminal; it emits a structured evidence packet (kind, failing criteria, worst margins) and an LLM *rework judge* routes the next attempt to `rework_criteria` (claim-preserving re-draft: the coverage guard re-runs against the persisted original claims - dropping a claim is rejected, the anti-goalpost floor) or `rework_config` (next simulator draft seeded with the judge's guidance). Judge fails open to escalate (never loops); budget = 2 rounds (`run_fly`), 0 for `run_new_story` (legacy rigid behavior). `fly_report.json` carries `rework.rounds` + directive trail so clean vs reworked landings stay separable. Live validation: scenario_23 re-landed after 2 rework rounds (4/4, attempt 3); scenario_24 landed clean on a single attempt (3/3, `rework.rounds 0`) - first flights proving the loop end-to-end. Suite 379 green. Tests: `tests/test_rework_loop.py`. |
| P8 ✅ | **Realizability guarantee** (branch `fix/realizability-p8`) - acceptance becomes a *guarantee*, not a hope (design: `REALIZABILITY.md`; evidence: findings_v4 addendum 3, the 21/22/25 re-fly). New envelope registry `syngen/packs/revops/envelope.py`: an outlier-faithful `pipeline_top_share` estimator (a sigma-only model under-predicted engine concentration 52% vs 96.9%) and a true `headline_growth_ceiling` (revenue is raked to plan, so growth is pinned); the growth ceiling is wired into `criteria_lint` as a HARD feasibility finding. Concentration is **not** hard-gated - its lever (outlier multiplier) is unbounded, so a sigma-only ceiling would false-kill reachable targets. Solver discipline (S21.2): `_autocalibrate_concentration` now solves WITHIN `sigma ≤ 4.0` and, when sigma alone cannot reach the target, raises the unbounded outlier multiplier - never writing an invalid config or dead-looping (the dead `found is None` branch and the domain-escaping 1.08 overshoot are gone). A latent null-param crash (`target_quarter_offset: null` → `int(None)`) in `_autocalibrate_coverage` was found and fixed repo-wide via a `_num` null-safe coercion (solver-discipline audit). Anti-vacuity (S25.3): thresholds that cannot fail (a `$1B` avg-price cap, `min_gap_pp=0`, one-sided bounds that exclude nothing) are HARD at Gate 1. Coherence (S22.1): a plan unit with no account potential is marked `structural`, not knob-reachable. Loop coverage (S25.4): a persistent Gate-1 `criteria_consistency` conflict routes through the bounded, claim-preserving rework judge (`rework.recover_criteria_consistency`) instead of dying terminal. Structure gate (S21.1): compares column *presence*, not order (an order-only mismatch could kill a genuinely landed flight). Suite 399 green; 20 new tests in `tests/test_p8_realizability.py`. Verification: targeted re-fly of scenarios 21/25; full 25-story certification is the weekend run. |
| P9.1 ✅ | **Drafter authoring guide ("skills")** (branch `feat/drafter-skills`) - the dominant remaining variance is the LLM drafter inventing different criteria each run (unreachable, mis-scoped). A curated doctrine fragment (`packs/revops/prompts/authoring_guide.txt`: scope only real units, match direction, no cannot-fail thresholds, stay within engine limits, one criterion per claim) plus facts generated from the signature registry (`PackTaxonomy.authoring_guide()`: valid coordinate spaces + `directional_params`/`pinned_quantity`) is injected as `{{authoring_guide}}` into the criteria drafter (`decompose.txt`) and the rework judge (`rework_judge.txt`). Prompt-level and probabilistic - reduces bad drafts, does not guarantee; the deterministic guarantee is P9.2 (next). Suite 402 green; 3 new tests in `tests/test_packs.py`. |
