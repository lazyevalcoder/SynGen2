...
**Status:** Iteration 5 scope REORDERED per the flight-model doctrine (see FLIGHT_MODEL.md). The aeronautics lens: SynGen is the airframe, the local LLM is the pilot, we are the aircraft makers - and v1's 25/25 was flown with an instructor's hands on the yoke (hand-edited session files in most landings). Revised priority: (1) autopilot/stall-recovery = autonomous remediation loop merging G3+G4 (classify failure -> remedy -> bounded retry -> structured escalation only as last resort); (2) `syngen fly` solo-certification harness + telemetry; (3) benchmark all 25 stories through fly -> unassisted landing rate becomes the M5 exit criterion; (4) F29 demoted below autonomy work (current vocabulary proved sufficient for all 25 v1 stories); (5) packaging. Success metrics change from "stories landed" to unassisted landing rate, escalations per 10 stories with classified root causes, and builder interventions per landing (target zero).
