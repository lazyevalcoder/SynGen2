...
def probe_criteria(client, story, use_skills=True, use_critic=True,
    ...
    Also carries the probe's llm_usage. A malformed draft or any phase
    error is captured as `error` with gate1_pass=False (never raises), so
    a harness can measure reliability instead of dying on one bad draft.
    """
    result = {"use_skills": use_skills, "gate1_pass": False}
    try:
        ...
        critic_blocks = []
        if use_critic:
            verdict = critique_artifact(client, story, "acceptance criteria",
                                        doc)
            critic_blocks = [render_issues([i])
                             for i in block_issues(verdict)]
