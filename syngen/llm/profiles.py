...
Parameter support on llama.cpp (probe 2026-09-11, Ornith-1.5-35B):
- enable_thinking via chat_template_kwargs: VERIFIED working (per-request);
  the way to turn thinking OFF.
- reasoning_budget_tokens=N: VERIFIED working - caps thinking at ~N tokens
  (~4-5 chars/token: 100->~0.5k chars, 250->~1.2k, 500->~2.3k). N=0 means
  UNLIMITED, not off. Ignored by hosted OpenAI-compatible APIs.
- reasoning_effort: IGNORED by llama.cpp (even "none" keeps thinking); kept
  only for OpenAI-compatible endpoints.
- response_format (json_object): NOT enforced by this build - the model still
  emitted fenced JSON, which extract_json already strips.
