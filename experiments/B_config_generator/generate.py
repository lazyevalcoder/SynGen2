...
def build_accounts(cfg, rng):
    ...
    suffixes = rng.choice(
        ["Group", "Holdings", "Systems", "Industries", "Labs", "Partners", "Logistics", "Technologies"],
        size=n,
    )
    return pd.DataFrame(
        {
            "account_id": [f"ACC-{i + 1:04d}" for i in range(n)],
            "account_name": [f"{i} {s} {j + 1:02d}" for j, (i, s) in enumerate(zip(industries, suffixes))],
...
def build_opportunities(cfg, accounts_df, rng):
    ...
    for qi, (label, q_end_str) in enumerate(zip(quarters, quarter_ends)):
        ...
        win_rate_q = spec["win_rate"] + rng.uniform(-spec["win_rate_jitter"], spec["win_rate_jitter"])
        ...
        base = np.array([dspec["base_by_quarter"][r][qi] for r in accts["region"]], dtype=float)
        ...
        realized_price = np.round(list_price * (1.0 - discount / 100.0), 2)
            ...
            rows.append(
                {
                    ...
                    "discount_pct": round(float(discount[j]), 2),
...
def main():
    config_path = sys.argv[1] if len(sys.argv) > 1 else "simulator.json"
    ...
    out_path = Path(cfg["output"]["workbook"])
    ...
    print(f"  opportunities: {len(opp_df)} rows")
